...
import jwt from "jsonwebtoken"; // Cria e válida o token JWT
import dotenv from "dotenv"; // Variáveis de ambiente do arquivo .env
import User from "./models/usuarioModel.js";

dotenv.config(); // Carrega as variáveis de um ambiente .env
...
app.get("/user/:id", checkToken, async (req, res) => {
  ...
  const user = await User.findById(id, "-passaword"); // Busca no banco o user sem a senha
...
app.post("/auth/login", async (req, res) => {
  ...
  if (!email) {
    return res.status(422).json({ msg: "O email deve ser infirmado!" });
  }
  ...
  const user = await User.findOne({ email: email }); // Busca o usuário no banco
...
mongoose
  .connect(
    `mongodb+srv://${dbUser}:${dbPassword}@projectapi.ve88g.mongodb.net/?retryWrites=true&w=majority&appName=ProjectAPI`
  )
  .then(() => {
    app.listen(3000);
